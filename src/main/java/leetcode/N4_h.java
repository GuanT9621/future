 ...
 *
 * 思路：
 * 看到复杂度为 O(log(m + n)) 时，就可以猜到算法应该是 二分查找法
 * 方法1： 使用归并的方式，合并两个有序数组，得到一个大的有序数组。大的有序数组的中间位置的元素，即为中位数。
 * 方法2： 不需要合并两个有序数组，只要找到中位数的位置即可。由于两个数组的长度已知，因此中位数对应的两个数组的下标之和也是已知的。
 *          维护两个指针，初始时分别指向两个数组的下标 0, 0 的位置，每次将指向较小值的指针后移一位
 *          （如果一个指针已经到达数组末尾，则只需要移动另一个数组的指针），直到到达中位数的位置。
 * 方法1，2都是 O(m + n)，我们需要简化到 O(log(m + n))
 * 优化：我们已知，如果 m + n 为奇数：中位数位于 (m + n) / 2；如果 m + n 为偶数：中位数位于 (m + n) / 2 - 1 , (m + n) / 2 + 1
 *      由于两个数组都是正序的，那么中位数意味着一半大于它，一半小于它。
 *      1 那么，如果数组 m 的最大值小于 n 的最小值，就很好处理
 *      2
 */
public class N4_h {
